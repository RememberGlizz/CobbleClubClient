...
public final class WardrobeState {
   ...
   public class_1799 previewStack(WardrobeSlot slot) {
      if (this.hidden) {
      ...
      if (this.presetPreview != null) {
         ...
         return entry != null ? this.stackFor(entry) : class_1799.field_8037;
      }
   }

   public class_1799 stackFor(WardrobeCosmeticEntry entry) {
      ...
      return (class_1799)this.stackCache.computeIfAbsent(key, (k) -> buildStack(entry, color));
   }

   private class_1799 stackForIdWithColor(String id, Integer color) {
      WardrobeCosmeticEntry entry = (WardrobeCosmeticEntry)this.catalog.get(id);
      if (entry == null) {
         return class_1799.field_8037;
      } else {
         Integer c = entry.getDyeable() ? color : null;
         return (class_1799)this.stackCache.computeIfAbsent(id + "#" + c, (k) -> buildStack(entry, c));
      }
   }

   private static class_1799 buildStack(WardrobeCosmeticEntry entry, Integer color) {
      class_2960 materialId = class_2960.method_12829(entry.getMaterial());
      if (materialId == null) {
         return class_1799.field_8037;
      } else {
         class_1792 item = (class_1792)class_7923.field_41178.method_10223(materialId);
         class_1799 stack = new class_1799(item);
         if (stack.method_7960()) {
            return class_1799.field_8037;
         } else {
            stack.method_57379(class_9334.field_49637, new class_9280(entry.getCustomModelData()));
            if (color != null) {
               stack.method_57379(class_9334.field_49644, new class_9282(color & 16777215, false));
            }
