...
public final class MapView {
   ...
   public int cursorBlockX(double mouseX) {
      return class_3532.method_15357(this.screenToWorldX(mouseX));
   }

   public int cursorBlockZ(double mouseY) {
      return class_3532.method_15357(this.screenToWorldZ(mouseY));
   }
   ...
   private static double clampAxis(double center, int min, int max, double viewSpanBlocks) {
      ...
      return viewSpanBlocks >= (double)(max - min) ? (double)(min + max) / (double)2.0F : class_3532.method_15350(center, (double)min + half, (double)max - half);
   }

   public void centerOnPlayer() {
      class_1657 player = class_310.method_1551().field_1724;
      if (player != null) {
         this.centerOn(player.method_23317(), player.method_23321());
      } else {
   ...
   public void zoomBy(int delta, double anchorMouseX, double anchorMouseY) {
      int next = class_3532.method_15340(this.zoomIndex + delta, 0, ZOOMS.length - 1);
   ...
   public void render(class_332 g, int mouseX, int mouseY, String selectedClaimId) {
      this.tiles.uploadIfDirty();
      this.clampCenter();
      this.requestVisibleTiles();
      g.method_25294(this.x0, this.y0, this.x1, this.y1, -16315620);
      g.method_44379(this.x0, this.y0, this.x1, this.y1);
   ...
   private void requestVisibleTiles() {
      int visMinCx = class_3532.method_15357(this.screenToWorldX((double)this.x0)) >> 4;
      int visMaxCx = class_3532.method_15357(this.screenToWorldX((double)this.x1)) >> 4;
      int visMinCz = class_3532.method_15357(this.screenToWorldZ((double)this.y0)) >> 4;
      int visMaxCz = class_3532.method_15357(this.screenToWorldZ((double)this.y1)) >> 4;
   ...
   private void drawVoidChecker(class_332 g) {
      int firstCx = class_3532.method_15357(this.screenToWorldX((double)this.x0)) >> 4;
      int firstCz = class_3532.method_15357(this.screenToWorldZ((double)this.y0)) >> 4;
      int lastCx = class_3532.method_15357(this.screenToWorldX((double)this.x1)) >> 4;
      int lastCz = class_3532.method_15357(this.screenToWorldZ((double)this.y1)) >> 4;
      ...
      for(int cx = firstCx; cx <= lastCx; ++cx) {
         ...
         for(int cz = firstCz; cz <= lastCz; ++cz) {
            if ((cx + cz & 1) != 0) {
               ...
               g.method_25294(sx0, sy0, sx1, sy1, -16052182);
            }
         }
      }
   ...
   private void drawAtlas(class_332 g) {
      ...
      g.method_51448().method_46416(screenX, screenY, 0.0F);
      g.method_51448().method_22905((float)this.zoom(), (float)this.zoom(), 1.0F);
      g.method_25290(this.tiles.location(), 0, 0, 0.0F, 0.0F, size, size, size, size);
      g.method_51448().method_22909();
   }

   private void drawGrid(class_332 g) {
      ...
      while(true) {
         ...
         if (sx >= this.x1) {
            ...
            while(true) {
               ...
               if (sx >= this.y0) {
                  ...
                  if (major || minors) {
                     g.method_25294(this.x0, sx, this.x1, sx + 1, major ? 1157627903 : 587202559);
                  }
               }
         ...
         if (sx >= this.x0) {
            boolean major = cx % 32 == 0;
            if (major || minors) {
               g.method_25294(sx, this.y0, sx + 1, this.y1, major ? 1157627903 : 587202559);
            }
         }
   ...
   private void drawClaims(class_332 g, int mouseX, int mouseY, String selectedClaimId) {
      ...
      if (pending != null) {
         ...
         int color = class_3532.method_15340(alpha, 64, 255) << 24 | 14723839;
         this.drawWorldOutline(g, pending.getMinCx() << 4, pending.getMinCz() << 4, pending.getMaxCx() + 1 << 4, pending.getMaxCz() + 1 << 4, color);
      }

   }

   private void drawSubClaims(class_332 g, String claimId) {
   ...
   private void drawSelectedTicks(class_332 g, MapClaimEntry claim) {
   ...
   private void drawCornerTicks(class_332 g, int sx0, int sy0, int sx1, int sy1) {
      int t = 4;
      int c = -1;
      g.method_25294(sx0 - 1, sy0 - 1, sx0 + t, sy0, c);
      g.method_25294(sx0 - 1, sy0 - 1, sx0, sy0 + t, c);
      g.method_25294(sx1 - t, sy0 - 1, sx1 + 1, sy0, c);
      g.method_25294(sx1, sy0 - 1, sx1 + 1, sy0 + t, c);
      g.method_25294(sx0 - 1, sy1, sx0 + t, sy1 + 1, c);
      g.method_25294(sx0 - 1, sy1 - t, sx0, sy1, c);
      g.method_25294(sx1 - t, sy1, sx1 + 1, sy1 + 1, c);
      g.method_25294(sx1, sy1 - t, sx1 + 1, sy1, c);
   }

   private void drawSelection(class_332 g) {
   ...
   private void drawClaimableRegion(class_332 g) {
   ...
   private void drawPlayerMarker(class_332 g) {
      class_1657 player = class_310.method_1551().field_1724;
      if (player != null) {
         float sx = (float)this.worldToScreenX(player.method_23317());
         float sy = (float)this.worldToScreenY(player.method_23321());
         int r = 5 + (int)Math.round((double)2.0F * Math.sin((double)System.currentTimeMillis() / (double)300.0F));
         g.method_49601((int)sx - r, (int)sy - r, 2 * r, 2 * r, 1627389951);
         g.method_51448().method_22903();
         g.method_51448().method_46416(sx, sy, 0.0F);
         g.method_51448().method_22907(class_7833.field_40718.rotationDegrees(player.method_36454() + 180.0F));
         g.method_25294(-1, -4, 2, -2, -16777216);
         g.method_25294(-2, -2, 3, 0, -16777216);
         g.method_25294(-3, 0, 4, 2, -16777216);
         g.method_25294(0, -3, 1, -2, -1);
         g.method_25294(-1, -2, 2, -1, -1);
         g.method_25294(-2, -1, 3, 1, -1);
         g.method_51448().method_22909();
      }
   }

   private void drawWorldRect(class_332 g, int wx0, int wz0, int wx1, int wz1, int fill, int outline) {
      this.drawWorldFill(g, wx0, wz0, wx1, wz1, fill);
      this.drawWorldOutline(g, wx0, wz0, wx1, wz1, outline);
   }

   private void drawWorldFill(class_332 g, int wx0, int wz0, int wx1, int wz1, int color) {
      ...
      g.method_25294(sx0, sy0, sx1, sy1, color);
   }

   private void drawWorldOutline(class_332 g, int wx0, int wz0, int wx1, int wz1, int color) {
      int sx0 = (int)Math.round(this.worldToScreenX((double)wx0));
      int sy0 = (int)Math.round(this.worldToScreenY((double)wz0));
      int sx1 = (int)Math.round(this.worldToScreenX((double)wx1));
      int sy1 = (int)Math.round(this.worldToScreenY((double)wz1));
      g.method_25294(sx0, sy0, sx1, sy0 + 1, color);
      g.method_25294(sx0, sy1 - 1, sx1, sy1, color);
      g.method_25294(sx0, sy0, sx0 + 1, sy1, color);
      g.method_25294(sx1 - 1, sy0, sx1, sy1, color);
   }
