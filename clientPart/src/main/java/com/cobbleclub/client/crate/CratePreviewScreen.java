...
public class CratePreviewScreen extends class_437 {
   ...
   private final Map<Integer, class_1799> stackCache = new HashMap();
   ...
   private final Map<String, class_1799> formStackCache = new HashMap();
   private final Map<class_1304, PrizeEntry> wornArmor = new EnumMap(class_1304.class);
      ...
      if (catalog.prizes != null) {
         ...
         for(int i = 0; i < catalog.prizes.size(); ++i) {
            ...
            if (p != null) {
               ...
               class_2561 label = PreviewUi.deserialize(p.displayName, pokemon ? p.species : p.material);
               class_2561 shinyLabel = p.shinyDisplayName != null ? PreviewUi.deserialize(p.shinyDisplayName, "") : null;
               Rarity r = (Rarity)scheme.get(p.chance);
               class_2561 paneSummary = class_2561.method_43470("Chance: " + oddsStr(p.chance));
               ...
               if (this.canTest) {
                  tooltip.add(class_2561.method_43470("Right-click to test reward").method_54663(-3116801));
                  if (p.shinyEligible) {
                     tooltip.add(class_2561.method_43470("Shift+right-click to test shiny reward").method_54663(-11702));
                  }
               }

               List<class_2561> shinyTooltip = null;
               if (shinyLabel != null) {
   ...
   private class_2561 shinyLabel() {
      return class_2561.method_43470(this.shiny ? "Shiny: On" : "Shiny: Off");
   }
   ...
   private void rebuildView() {
      ...
      if (this.selectedEntry >= 0) {
         ...
         if (rowOf < this.scrollRow) {
            ...
            this.scrollRow = class_3532.method_15340(rowOf - this.visibleRows() + 1, 0, this.maxScrollRow());
         }
      }
   ...
   private class_1799 stackFor(PrizeEntry e) {
      return (class_1799)this.stackCache.computeIfAbsent(e.index(), (k) -> {
         class_1799 stack;
         if (e.pokemon()) {
         ...
         if (!stack.method_7960()) {
            stack.method_7939(class_3532.method_15340(e.amount(), 1, 99));
         }
   ...
   private class_2561 labelFor(PrizeEntry e) {
      return this.shiny && e.shinyLabel() != null ? e.shinyLabel() : e.label();
   }

   private List<class_2561> tooltipFor(PrizeEntry e) {
   ...
   private class_1799 formStack(PokemonFormCatalog.FormEntry f) {
      String var10000 = f.getSpecies();
      String key = var10000 + "|" + String.join(",", f.getAspects()) + (this.shiny ? "|shiny" : "");
      return (class_1799)this.formStackCache.computeIfAbsent(key, (k) -> PokemonPreviewRenderer.modelStack(f.getSpecies(), this.shiny ? withShiny(f.getAspects()) : f.getAspects()));
   }

   private class_2561 formLabel(PokemonFormCatalog.FormEntry f) {
      return class_2561.method_43470(this.shiny ? "Shiny " + f.getLabel() : f.getLabel());
   }
   ...
   private class_1304 armorSlotFor(PrizeEntry e) {
      if (!e.pokemon() && e.wornSlot() == null) {
         class_5151 equipable = class_5151.method_48957(this.stackFor(e));
         return equipable != null && equipable.method_7685().method_5925() == class_1305.field_6178 ? equipable.method_7685() : null;
      } else {
         return null;
      }
   }
   ...
   private int visibleRows() {
      ...
      return class_3532.method_15340(this.gridRows.size(), 1, Math.max(1, Math.min(9, fit)));
   }
   ...
   protected void method_25426() {
      ...
      this.search = new class_342(this.field_22793, this.gridX(), this.searchY(), 180, 12, class_2561.method_43470("Search"));
      this.search.method_47404(class_2561.method_43470("Search prizes…"));
      ...
      if (this.hasShinyToggle) {
         this.method_37063(new ThemedButton(this.panelLeft() + 384 - 140, footerY, 74, 20, this.shinyLabel(), (b) -> {
            this.shiny = !this.shiny;
            this.stackCache.clear();
            this.rebuildView();
            b.method_25355(this.shinyLabel());
         }));
      }

      this.method_37063(new ThemedButton(this.panelLeft() + 384 - 60, footerY, 52, 20, class_2561.method_43470("Close"), (b) -> this.method_25419()));
   }
   ...
   public void method_25394(class_332 g, int mouseX, int mouseY, float partialTick) {
      ...
      g.method_25294(left - 1, top - 1, left + 384 + 1, top + 252 + 1, -16447985);
      g.method_25296(left, top, left + 384, top + 252, -199614136, -200601562);
      g.method_49601(left, top, 384, 252, -13747610);
      g.method_25296(left, top, left + 384, top + 18, -14405538, -15459782);
      g.method_25294(left, top + 18, left + 384, top + 19, this.accent);
      g.method_27534(this.field_22793, this.method_25440(), left + 192, top + 5, -2053377);
      ...
      class_1304 shownArmorSlot = shown != null ? this.armorSlotFor(shown) : null;
      if (shownArmorSlot == null && (shown != null || this.view.isEmpty())) {
         if (shown != null) {
            if (shown.wornSlot() != null) {
               class_1799 stack = this.stackFor(shown);
               WardrobeSlot slot = shown.wornSlot();
               this.wornPreview.updateEquipment(slot == WardrobeSlot.HELMET ? stack : class_1799.field_8037, slot == WardrobeSlot.BACKPACK ? stack : class_1799.field_8037, slot == WardrobeSlot.BALLOON ? stack : class_1799.field_8037);
         ...
         Map<class_1304, class_1799> equipment = new EnumMap(class_1304.class);
      ...
      if (shown != null) {
         g.method_51439(this.field_22793, shown.paneSummary(), px0, py1 + 4, oddsArgb(shown.rarity()), false);
      }
      ...
      if (!forms.isEmpty()) {
         ...
         for(int i = 0; i < forms.size(); ++i) {
            ...
            g.method_25294(p[0], p[1], p[0] + 20 - 1, p[1] + 20 - 1, -15064506);
            if (sel) {
               g.method_25294(p[0], p[1], p[0] + 20 - 1, p[1] + 20 - 1, 1090519039);
            }

            int border = !sel && i != this.hoveredForm ? -13747610 : -1;
            g.method_49601(p[0], p[1], 19, 19, border);
            g.method_51427(this.formStack((PokemonFormCatalog.FormEntry)forms.get(i)), p[0] + 2, p[1] + 2);
         }
      }
      ...
      if (this.hoveredEntry >= 0 && this.hoveredEntry < this.view.size()) {
         GuiDepth.clearForOverlay(g);
         Tooltips.render(g, this.field_22793, this.tooltipFor((PrizeEntry)this.view.get(this.hoveredEntry)), mouseX, mouseY);
      } else if (this.hoveredForm >= 0 && this.hoveredForm < forms.size() && !this.dragging) {
         GuiDepth.clearForOverlay(g);
         Tooltips.render(g, this.field_22793, this.formLabel((PokemonFormCatalog.FormEntry)forms.get(this.hoveredForm)), mouseX, mouseY);
      }
   ...
   private void renderGrid(class_332 g, int mouseX, int mouseY) {
      ...
      for(int r = 0; r < rows; ++r) {
         ...
         for(int col = 0; col < line.prizes().length; ++col) {
            ...
            g.method_25294(cx, ly, cx + 20 - 1, ly + 20 - 1, -15064506);
            if (sel) {
               g.method_25294(cx, ly, cx + 20 - 1, ly + 20 - 1, 1090519039);
            }

            int border = sel ? -1 : (hov ? -1 : e.rarity().color());
            if (border != 0) {
               g.method_49601(cx, ly, 19, 19, border);
            }

            class_1799 icon = this.stackFor(e);
            g.method_51427(icon, cx + 2, ly + 2);
            g.method_51431(this.field_22793, icon, cx + 2, ly + 2);
         }
      }
   ...
   public boolean method_25402(double mouseX, double mouseY, int button) {
      ...
      if (clicked < 0) {
         ...
         if (button == 1 && this.canTest) {
            ...
            boolean shiny = method_25442() && e.shinyEligible();
            CratePreviewNetworking.requestTestReward(this.crateId, e.index(), shiny);
         } else {
            PrizeEntry e = (PrizeEntry)this.view.get(clicked);
            class_1304 slot = this.armorSlotFor(e);
   ...
   public boolean method_25401(double mouseX, double mouseY, double scrollX, double scrollY) {
      if (this.inPreview(mouseX, mouseY)) {
         this.zoom = class_3532.method_15363(this.zoom * (scrollY > (double)0.0F ? 1.12F : 0.89285713F), 0.5F, 2.6F);
         this.lastInteractMs = class_156.method_658();
         return true;
      } else {
         ...
         if (max <= 0 || !this.inGrid(mouseX, mouseY) && !this.onScrollbar(mouseX, mouseY)) {
            ...
            this.scrollRow = class_3532.method_15340(this.scrollRow - (int)Math.signum(scrollY), 0, max);
            return true;
         }
      }
   }
   ...
   @Environment(EnvType.CLIENT)
   private static record PrizeEntry(class_2561 label, class_2561 shinyLabel, boolean pokemon, String species, List<String> aspects, String material, int customModelData, int shinyCustomModelData, boolean shinyEligible, int amount, int index, Rarity rarity, class_2561 paneSummary, List<class_2561> tooltip, List<class_2561> shinyTooltip, WardrobeSlot wornSlot, List<String> evolutions) {
   }
