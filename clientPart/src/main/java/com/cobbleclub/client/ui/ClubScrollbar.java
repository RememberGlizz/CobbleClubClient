...
public final class ClubScrollbar {
   ...
   public static void draw(class_332 g, int x, int y0, int y1, int scroll, int maxScroll) {
      draw(g, x, y0, y1, scroll, maxScroll, y1 - y0);
   }

   public static void draw(class_332 g, int x, int y0, int y1, int scroll, int maxScroll, int visible) {
      if (maxScroll > 0) {
         int trackH = y1 - y0;
         g.method_25294(x, y0, x + 3, y1, -15986650);
         int thumbH = thumbH(trackH, maxScroll, visible);
         int thumbY = y0 + (trackH - thumbH) * scroll / maxScroll;
         g.method_25294(x, thumbY, x + 3, thumbY + thumbH, -10862962);
      }
   }
   ...
   public static int scrollAt(double my, int y0, int y1, int maxScroll, int visible) {
      if (maxScroll <= 0) {
         ...
         return travel <= 0 ? 0 : class_3532.method_15340((int)Math.round((my - (double)y0 - (double)thumbH / (double)2.0F) * (double)maxScroll / (double)travel), 0, maxScroll);
      }
   }
