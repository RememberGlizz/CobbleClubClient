...
public abstract class LivingEntityRendererMixin<
    ...
    @WrapOperation(
            ...
            at = @At(
                    value = "INVOKE",
                    target = "Lnet/minecraft/class_4597;getBuffer(Lnet/minecraft/class_1921;)Lnet/minecraft/class_4588;",
                    remap = false
            ),
            remap = false
    )
    private VertexConsumer cobbleclub$applyPokemonShader(
        ...
        try {
            ...
            Optional<String> aspect = pokemon.getAspects()
                    .stream()
            ...
            Identifier texture = VaryingModelRepository.INSTANCE.getTexture(
                    pokemon.getPokemon().getSpecies().getResourceIdentifier(),
                    (PosableState) delegate
            );

            return buffer.getBuffer(
                    CobbleClubRenderTypes.pokemonShader(
                            texture,
                            aspect.get()
                    )
            );
