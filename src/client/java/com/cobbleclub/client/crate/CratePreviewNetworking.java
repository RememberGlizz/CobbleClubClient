...
public final class CratePreviewNetworking {
   ...
   public static void init() {
      ClientPlayNetworking.registerGlobalReceiver(Payloads.CratePreview.ID, (payload, context) -> {
         try {
            CrateCatalog catalog = GSON.fromJson(payload.json(), CrateCatalog.class);
            ...
            if (totalParts == 1) {
               reset();
               context.client().setScreen(new CratePreviewScreen(catalog));
               return;
            }
            ...
            if (catalog.prizes != null) pendingPrizes.addAll(catalog.prizes);
            if (catalog.part == totalParts - 1) {
               pending.prizes = pendingPrizes;
               CrateCatalog assembled = pending;
               reset();
               context.client().setScreen(new CratePreviewScreen(assembled));
            }
   ...
   static void requestTestReward(String crateId, int prizeIndex, boolean shiny) {
      if (crateId != null && ClientPlayNetworking.canSend(Payloads.CrateTestReward.ID)) {
         ClientPlayNetworking.send(new Payloads.CrateTestReward(crateId, prizeIndex, shiny));
      }
   }
}
