...
public final class ClaimListPanel {
    ...
    private List<Row> rows() {
        ArrayList<Row> claimRows = new ArrayList<Row>();
        for (ClaimDetailEntry claim : this.claims()) {
            claimRows.add(new Row(claim, null, 30, ClaimsState.standingInside(claim)));
        }
        claimRows.sort(Comparator.comparing(rowx -> !rowx.standing()));
        ArrayList<Row> rows = new ArrayList<Row>();
        for (Row row : claimRows) {
            rows.add(row);
            if (row.claim().getSubClaims() == null) continue;
    ...
    public void render(DrawContext g, int mouseX, int mouseY, String selectedClaimId) {
        ...
        this.scroll = MathHelper.clamp((int)this.scroll, (int)0, (int)this.maxScroll(rows));
        String header = (trustedTab ? "Trusted Claims (" : "Your Claims (") + this.claims().size() + ")";
        g.drawTextWithShadow(this.font, (Text)Text.literal((String)header).formatted(Formatting.GRAY), this.x0 + 8, this.y0 + 5, -7035976);
        g.fill(this.x0 + 1, this.listTop() - 1, this.x1 - 1, this.listTop(), -13747610);
        if (rows.isEmpty()) {
            g.drawCenteredTextWithShadow(this.font, (Text)Text.literal((String)(trustedTab ? "No trusted claims" : "No claims yet")), (this.x0 + this.x1) / 2, (this.y0 + this.y1) / 2 - 10, -7035976);
            g.drawCenteredTextWithShadow(this.font, (Text)Text.literal((String)(trustedTab ? "Claims others trust you in appear here" : "Claim land on the Map tab")).formatted(Formatting.GRAY), (this.x0 + this.x1) / 2, (this.y0 + this.y1) / 2 + 2, -7035976);
        } else {
            ...
            for (Row row : rows) {
                if (y + row.height() >= this.listTop() && y <= this.y1) {
                    boolean hover;
                    boolean bl = hover = this.contains(mouseX, mouseY) && mouseY >= Math.max(y, this.listTop()) && mouseY < y + row.height();
                    if (row.sub() == null) {
                        ...
                        g.drawTextWithShadow(var10001, (Text)Text.literal((String)("\u2514 " + var10002)).formatted(Formatting.GRAY), this.x0 + 18, y + 4, hover ? -1 : -7035976);
                        g.disableScissor();
                    }
                }
                y += row.height();
            ...
            if (!trustedTab && this.maxScroll(rows) == 0 && y + 26 < this.y1) {
                g.drawCenteredTextWithShadow(this.font, (Text)Text.literal((String)"Shift+drag on the Map tab to claim more land").formatted(Formatting.GRAY), (this.x0 + this.x1) / 2, y + 12, -7035976);
            }
            g.disableScissor();
    ...
    private void renderClaimRow(DrawContext g, Row row, int y, boolean hover, boolean selected) {
        ...
        int n = subs = claim.getSubClaims() != null ? claim.getSubClaims().size() : 0;
        String statsMeta = w + "x" + d + (String)(subs > 0 ? " \u00b7 " + subs + (subs == 1 ? " sub" : " subs") : "");
        ...
        g.drawTextWithShadow(this.font, (Text)Text.literal((String)claim.getName()), this.x0 + 10, y + 5, -1);
        if (row.standing()) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollY) {
        ...
        this.scroll = MathHelper.clamp((int)(this.scroll - (int)(scrollY * 30.0)), (int)0, (int)this.maxScroll(this.rows()));
        return true;
    }

    @Environment(value=EnvType.CLIENT)
    private record Row(ClaimDetailEntry claim, SubClaimEntry sub, int height, boolean standing) {
    }
}
