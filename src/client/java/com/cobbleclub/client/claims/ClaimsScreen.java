...
public class ClaimsScreen extends Screen {
    ...
    public Element getFocused() {
        ...
        return box != null ? box : super.getFocused();
    }
    ...
    protected void init() {
        ...
        if (this.state.mapEnabled) {
            this.mapTabButton = this.addDrawableChild(new ThemedButton(tabX, this.panelTop() + 2, this.tabWidth("Map"), 20, Text.literal("Map"), (b) -> this.switchTab(ClaimsScreen.Tab.MAP)));
        ...
        this.centerButton = this.addDrawableChild(new ThemedButton(0, 0, 52, 16, Text.literal("Center"), (b) -> this.mapView.centerOnPlayer()));
        if (!this.state.helpLines.isEmpty()) {
            this.helpButton = this.addDrawableChild(new ThemedButton(0, this.panelTop() + 2, 20, 20, Text.literal("?"), (b) -> this.toggleHelp()));
            this.helpButton.setTooltip(Tooltip.of(this.state.screenText("help_title")));
        }

        this.confirmButton = this.addDrawableChild(new ThemedButton(0, 0, 24, 18, Text.literal("✔"), ThemedButton.Variant.GREEN, (b) -> this.confirmSelection()));
        this.confirmButton.setTooltip(Tooltip.of(Text.literal("Confirm selection (Enter)")));
        this.cancelButton = this.addDrawableChild(new ThemedButton(0, 0, 24, 18, Text.literal("✕"), ThemedButton.Variant.RED, (b) -> this.mapView.selection().clear()));
        this.cancelButton.setTooltip(Tooltip.of(Text.literal("Cancel selection (Esc)")));
        this.detailPanel.createWidgets(widget -> this.addDrawableChild(widget));
    }
    ...
    private void renderActionBar(DrawContext g) {
        ...
        if (!selection.isActive()) {
            ...
            String sizeCost = selection.widthChunks() + "x" + selection.depthChunks() + " Chunks · " + (selection.isResize() && cost >= 0 ? "+" : "") + cost + " Blocks";
            String label = (selection.isResize() ? "Resizing Claim: " : "New Claim: ") + sizeCost;
            String issueName = String.valueOf(this.mapView.selectionIssue());
            String issueText;
            if ("OVERLAP".equals(issueName)) {
                issueText = " — overlaps a claim";
            } else if ("TOO_EXPENSIVE".equals(issueName)) {
                issueText = " — not enough blocks";
            } else if ("TOO_LARGE".equals(issueName)) {
                issueText = " — too large";
            } else if ("TOO_FAR".equals(issueName)) {
                issueText = " — too far from you";
            } else {
                issueText = "";
            }
