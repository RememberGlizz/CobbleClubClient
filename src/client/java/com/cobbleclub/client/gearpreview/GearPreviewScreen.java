...
public class GearPreviewScreen
extends Screen {
    ...
    public GearPreviewScreen(List<GearCatalogPayload.GearCatalogSet> sets) {
        super((Text)Text.literal((String)"Gear Preview"));
        this.sets = sets;
        this.tabLabels = GearPreviewScreen.deserializeTabLabels(sets);
        PreviewUi.playOpen();
    }
    ...
    private static List<Text> deserializeTabLabels(List<GearCatalogPayload.GearCatalogSet> sets) {
        ...
        ArrayList<Text> labels = new ArrayList<Text>(sets.size());
        for (GearCatalogPayload.GearCatalogSet set : sets) {
            MutableText label = null;
            if (registry != null) {
                try {
                    label = Text.Serialization.fromJson((String)set.displayName(), (RegistryWrapper.WrapperLookup)registry);
                }
                catch (Exception exception) {
                    // empty catch block
                }
            }
            labels.add((Text)(label != null ? label : Text.literal((String)set.id())));
        }
        return labels;
    }

    private List<TabRect> computeTabRects() {
        ArrayList<TabRect> rects = new ArrayList<TabRect>(this.sets.size());
    ...
    private int tabRows() {
        return this.tabRects.isEmpty() ? 1 : (this.tabRects.get(this.tabRects.size() - 1).y() - (this.panelTop() + 22)) / 18 + 1;
    }
    ...
    private List<ItemStack> activeItems() {
        return this.sets.isEmpty() ? List.of() : this.sets.get(this.activeSet).items();
    }

    private int visibleRows() {
        int rows = (this.activeItems().size() + 9 - 1) / 9;
        return MathHelper.clamp((int)rows, (int)1, (int)6);
    }
    ...
    protected void init() {
        ...
        this.addDrawableChild(new ThemedButton(this.panelLeft() + 456 - 118, this.panelTop() + 252 - 24, 52, 20, (Text)Text.literal((String)"Reset"), b -> {
        ...
        this.addDrawableChild(new ThemedButton(this.panelLeft() + 456 - 60, this.panelTop() + 252 - 24, 52, 20, (Text)Text.literal((String)"Close"), b -> this.close()));
    }

    private Text hidePlayerLabel() {
        return Text.literal((String)(this.hidePlayer ? "Show Player" : "Hide Player"));
    }

    private static EquipmentSlot slotFor(ItemStack stack) {
        Equipment equipable = Equipment.fromStack((ItemStack)stack);
        return equipable != null ? equipable.getSlotType() : EquipmentSlot.MAINHAND;
    }

    private Map<EquipmentSlot, ItemStack> resolvedEquipment() {
        EnumMap<EquipmentSlot, ItemStack> resolved = new EnumMap<EquipmentSlot, ItemStack>(this.overrides);
        if (this.hovered != null) {
            resolved.put(GearPreviewScreen.slotFor(this.hovered), this.hovered);
        }
        return resolved;
    }
    ...
    public void render(DrawContext guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        this.hovered = this.dragging ? null : this.hoveredItem(mouseX, mouseY);
        ...
        if (this.hidePlayer) {
            ...
            if (focus == null || focus.isEmpty()) {
                focus = this.resolvedEquipment().getOrDefault(EquipmentSlot.MAINHAND, ItemStack.EMPTY);
            }
        ...
        if (this.inPreview(mouseX, mouseY)) {
            guiGraphics.drawCenteredTextWithShadow(this.textRenderer, (Text)Text.literal((String)"Drag \u2022 Scroll to zoom"), (px0 + px1) / 2, py1 - 11, -1063348548);
        }
    ...
    private void renderGrid(DrawContext guiGraphics, int mouseX, int mouseY) {
        ...
        for (int i = first = this.scrollRow * 9; i < items.size() && i < first + 9 * rows; ++i) {
            ...
            if (mouseX < cellX || mouseX >= cellX + 26 || mouseY < cellY || mouseY >= cellY + 26) continue;
    ...
    private ItemStack hoveredItem(double mouseX, double mouseY) {
        ...
        if (!(mouseX < (double)x || mouseX >= (double)(x + 234) || mouseY < (double)y || mouseY >= (double)(y + this.visibleRows() * 26))) {
            int col = (int)((mouseX - (double)x) / 26.0);
            int row = (int)((mouseY - (double)y) / 26.0);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        for (TabRect tab : this.tabRects) {
            if (!(mouseX >= (double)tab.x()) || !(mouseX < (double)(tab.x() + tab.w())) || !(mouseY >= (double)tab.y()) || !(mouseY < (double)(tab.y() + 16))) continue;
        ...
        if (clicked != null) {
            EquipmentSlot slot = GearPreviewScreen.slotFor(clicked);
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (this.dragging) {
            if (this.hidePlayer) {
                this.itemOrientation.premul((Quaternionfc)new Quaternionf().rotationY((float)Math.toRadians(dragX)).rotateX((float)Math.toRadians(-dragY)));
            } else {
                this.yaw += (float)dragX;
                this.pitch = MathHelper.clamp((float)(this.pitch - (float)dragY), (float)-20.0f, (float)20.0f);
            }
            this.lastInteractMs = Util.getMeasuringTimeMs();
            return true;
        }
    ...
    @Environment(value=EnvType.CLIENT)
    private record TabRect(int index, int x, int y, int w) {
    }
}
